...
void SparseMatrix ::getGraphviz()
{
    ...
    if (temp != NULL)
    {
        fprintf(fp, "Mt -> node%s%s%s;\n", getVal(temp->row).c_str(), getVal(temp->col).c_str(), getVal(temp->val).c_str());
    }
    while (temp != NULL)
    {
        fprintf(
        ...
        if (temp->down != NULL)
        {
            fprintf(
                ...
                getVal(temp->row).c_str(), getVal(temp->col).c_str(),
                getVal(temp->val).c_str(), getVal(temp->down->row).c_str(),
                getVal(temp->down->col).c_str(), getVal(temp->down->val).c_str());
        }
    ...
    while (temp != NULL)
    {
        additionalTextCol += "node" + getVal(temp->row) + getVal(temp->col) + getVal(temp->val) + "; ";
        fprintf(
            fp,
            "node%s%s%s [label=\"C%d \" width = 1.5 style = filled, fillcolor = lightskyblue, group = %d ];\n",
            getVal(temp->row).c_str(), getVal(temp->col).c_str(),
            getVal(temp->val).c_str(), temp->col, count);
        ...
        if (temp->right != NULL)
        {
            fprintf(
                fp, "node%s%s%s -> node%s%s%s;\n",
                getVal(temp->row).c_str(), getVal(temp->col).c_str(),
                getVal(temp->val).c_str(), getVal(temp->right->row).c_str(),
                getVal(temp->right->col).c_str(), getVal(temp->right->val).c_str());
        }
    ...
    while (temp != NULL)
    {
        ...
        while (temp2 != NULL)
        {
            fprintf(
                ...
                getVal(temp2->val).c_str(), temp2->val, temp2->col + 1000);
    ...
    while (temp != NULL)
    {
        ...
        while (temp2 != NULL)
        {
            additionalTxt += "node" + getVal(temp2->row) + getVal(temp2->col) + getVal(temp2->val) + "; ";
            // right relationships
            if (temp2->right != NULL)
            {
                fprintf(
                    fp, "node%s%s%s -> node%s%s%s;\n",
                    getVal(temp2->row).c_str(), getVal(temp2->col).c_str(),
                    getVal(temp2->val).c_str(), getVal(temp2->right->row).c_str(),
                    getVal(temp2->right->col).c_str(), getVal(temp2->right->val).c_str());
            }
            // up relationships
            if (temp2->up != NULL && count > 0)
            {
                fprintf(
                    ...
                    getVal(temp2->up->val).c_str(), getVal(temp2->row).c_str(),
                    getVal(temp2->col).c_str(), getVal(temp2->val).c_str());
            }
